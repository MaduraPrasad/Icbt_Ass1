...
const registerController = async (req, res) => {
  try {
    const exisitingUser = await newUserModel.findOne({ email: req.body.email });
    if (exisitingUser) {
      return res
        .status(200)
...
const authController = async (req, res) => {
  try {
    const user = await newUserModel.findById({ _id: req.body.userId });
...
// APpply Consultant CTRL
const applyConsultantController = async (req, res) => {
  try {
    const newConsultant = await consultantModel({ ...req.body, status: "pending" });
    await newConsultant.save();
    const adminUser = await newUserModel.findOne({ isAdmin: true });
    const notifcation = adminUser.notifcation;
    notifcation.push({
      type: "apply-consultant-request",
      message: `${newConsultant.firstName} ${newConsultant.lastName} Has Applied For A newConsultant Account`,
      data: {
    ...
    await newUserModel.findByIdAndUpdate(adminUser._id, { notifcation });
    res.status(201).send({
      success: true,
...
const getAllNotificationController = async (req, res) => {
  try {
    const user = await newUserModel.findOne({ _id: req.body.userId });
...
const bookeAppointmnetController = async (req, res) => {
  try {
    ...
    const newAppointment = new shedullingModel(req.body);
    await newAppointment.save();
    const user = await newUserModel.findOne({ _id: req.body.consultantInfo.userId });
